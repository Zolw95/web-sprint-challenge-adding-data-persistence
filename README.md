...
1. Explain the difference between `Relational Databases` and `SQL`.
   SQL is a language designed for managing data held in relational databases and relational databases are the actual storage.
2. Why do tables need a `primary key`?
   Tables need a primary key (generated by the database) to identify each row/record. Primary keys must be unique.

3. What is the name given to a table column that references the primary key on another table.
   It's called the foreign key.

4. What do we need in order to have a _many to many_ relationship between two tables.
   We need to create a separate (bridging) table that includes foreign keys(references) for both tables.
